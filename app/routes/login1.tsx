import {
  ...
  export default function Login() {
    ...
    const [selectedLoginType, setSelectedLoginType] = useState<"email" | "password">("password");
          ...
          <Form method="post" onSubmit={submitForm}>
            ...
            <CardBody className="pb-0 flex flex-col">
              <Input
                ...
                isRequired
                // isInvalid={isInvalid}
                // color={isInvalid ? "danger" : email === "" ? "default" : "success"}
                endContent={<Mail />}
                label="邮箱"
                type="email"
                name="email"
              ...
              {isForgetPassword || isRegister ? (
                <>
                  <div className="flex flex-row items-center space-x-2">
                    <Input
                      ...
                      variant="flat"
                    />
                    {/* <Button color="primary" isLoading={!isFinished} type="submit" name="intent" value={"sendCode" + isRegister ? "Register" : "Reset"} isDisabled={!isFinished || isInvalid || isSendingEmail || email == ""} onClick={() => { */}
                        <Button color="primary" isLoading={!isFinished} type="submit" name="intent" value={"sendCode" + isRegister ? "Register" : "Reset"} isDisabled={!isFinished || isSendingEmail || email == ""} onClick={() => {
                      setIsSendingEmail(true);
                      setIsFirstTimeSend(false);
                      submitEmail({
                        email,
                      },isRegister ? "sendCodeRegister" : "sendCodeReset");
                    }}>
                  ...
                  {isRegister ? (
                    <>
                      ...
                      <Input
                        ...
                        color={isInvalidUsername ? "danger" : username === "" ? "default" : "success"}
                        errorMessage="* 用户名长度应为2-20位，仅支持中英文、数字和部分标点（_-.）"
                        description="* 用户名长度应为2-20位，仅支持中英文、数字和部分标点（_-.）"
                        endContent={
                          <User />
                        }
                      />
                  ...
                  <Input
                    ...
                    color={isInvalidPassword ? "danger" : password === "" ? "default" : "success"}
                    endContent={
                    ...
                    name="password"
                    placeholder={"请输入你的" + (isForgetPassword ? "新密码" : "密码")}
                    description="* 密码长度应为8-20位，并包含大小字母和数字，以及部分特殊字符"
                    errorMessage="* 密码长度应为8-20位，并包含大小写字母和数字，以及部分特殊字符"
                    variant="bordered"
                  />
                  <Input
                    ...
                    color={isInvalidConfirmPassword ? "danger" : confirmPassword === "" ? "default" : "success"}
                    endContent={
                      <div className="flex items-center">
                        <Button isIconOnly variant="light" size="sm" onClick={() => setConfirmPasswordVisible(prev => !prev)}>
                    ...
                    placeholder={"请再次输入你的" + (isForgetPassword ? "新密码" : "密码")}
                ...
                //   onSelectionChange={key => setSelectedLoginType(key as "email" | "password")}
                //   color="primary"
                // >
                //   <Tab key="email" title="使用验证码" className="w-full mx-0 px-0">
                ...
                //         variant="flat"
                //       />
                //       <Button color="primary" isLoading={!isFinished} type="submit" name="intent" value="sendCodeLogin" isDisabled={!isFinished || isInvalid || isSendingEmail || email == ""} onClick={() => {
                //         setIsSendingEmail(true);
                //         setIsFirstTimeSend(false);
                //         submitEmail({
                //           email
                ...
                //     </div>
                //   </Tab>
                //   <Tab key="password" title="使用密码" className="w-full mx-0 px-0">
                    <Input
                      value={password}
                      onValueChange={setPassword}
                      isRequired
              ...
              {isRegister ? (
                ...
                <div className="flex flex-col md:flex-row md:items-center items-end space-x-4">
                  ...
                  <Button color="primary" type="submit" name="intent" value={selectedLoginType + "Login"} isDisabled={
                    email === "" || password === "" && selectedLoginType=== "password"//|| isInvalid 
                  }>
                    登录
                  </Button>
                </div>
